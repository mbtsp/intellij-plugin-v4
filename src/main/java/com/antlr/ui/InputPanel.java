...
import com.antlr.language.psrsing.PreviewParser;
import com.antlr.preview.PreviewState;
import com.antlr.ui.view.PreviewEditorMouseListener;
import com.antlr.ui.view.PreviewPanel;
import com.antlr.ui.view.WrappedFlowLayout;
...
import static com.antlr.util.AntlrUtilKt.*;
...
public class InputPanel {
    ...
    public void createManualInputPreviewEditor(final PreviewState previewState) {
        ...
        Editor editor = createPreviewEditor(previewState.getGrammarFile(), doc, false);
        ...
        ApplicationManager.getApplication().runWriteAction(() -> doc.setText(previewState.getManualInputText()));
    }
    ...
    public void selectFileEvent() {
        ...
        VirtualFile inputFile = previewState.getInputFile();
        if (inputFile == null) {
        ...
        VirtualFile currentGrammarFile = getCurrentEditorFileG4(previewState.getProject());
    ...
    public void grammarFileSaved() {
        clearParseErrors();
        if (previewState != null && previewState.getStartRuleName() != null) {
            setStartRuleName(previewState.getGrammarFile(), previewState.getStartRuleName());
        } else {
            resetStartRuleLabel();
        }
    }
    ...
    public void switchToGrammar(PreviewState previewState, VirtualFile grammarFile) {
        ...
        if (previewState.getStartRuleName() != null) {
            setStartRuleName(grammarFile, previewState.getStartRuleName());
        } else {
            resetStartRuleLabel();
        }
    }
...
//        uninstallListeners(previewState.getInputEditor());
    ...
    public void showTokenInfoUponCtrlKey(Editor editor, PreviewState previewState, int offset) {
        if (previewState == null || previewState.getG() == null || previewState.parsingResult == null) {
            return;
        }
        Token tokenUnderCursor = ParsingUtils.INSTANCE.getTokenUnderCursor(previewState, offset);
        if (tokenUnderCursor == null) {
            ...
            tokenUnderCursor = ParsingUtils.INSTANCE.getSkippedTokenUnderCursor(tokenStream, offset);
        }
        ...
        String tokenInfo =
                String.format("#%d Type %s, Line %d:%d%s",
                        ...
                        previewState.getG().getTokenDisplayName(tokenUnderCursor.getType()),
    ...
    public void showParseRegion(Editor editor, PreviewState previewState, int offset) {
        ...
        TerminalNode nodeWithToken =
                (TerminalNode) ParsingUtils.INSTANCE.getParseTreeNodeWithToken(tree, tokenUnderCursor);
    ...
    public void setCursorToGrammarElement(Project project, PreviewState previewState, int offset) {
        ...
        PreviewParser parser = (PreviewParser) previewState.parsingResult.parser;
        if (parser.getInputTokenToStateMap() == null) {
            return;
        }
        Integer atnState = parser.getInputTokenToStateMap().get(tokenUnderCursor);
        if (atnState == null) { // likely an error token
            //LOG.error("no ATN state for input token " + tokenUnderCursor);
            return;
        }

        Interval region = previewState.getG().getStateToGrammarRegion(atnState);
        CommonToken token =
                (CommonToken) previewState.getG().tokenStream.get(region.a);
    ...
    public void setCursorToGrammarRule(Project project, PreviewState previewState, int offset) {
        ...
        Rule rule = previewState.getG().getRule(ruleIndex);
    ...
    public void jumpToGrammarPosition(Project project, int start) {
        ...
        final Editor grammarEditor = getEditor(previewState.getGrammarFile(), project);
    ...
    public static void showTooltips(Editor editor, @NotNull PreviewState previewState, int offset) {
        ...
        List<RangeHighlighter> highlightersAtOffset = getRangeHighlightersAtOffset(editor, offset);

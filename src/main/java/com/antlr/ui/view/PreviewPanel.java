...
import com.antlr.actions.CancelParserAction;
...
import com.antlr.listener.PreViewListener;
...
public class PreviewPanel extends JPanel implements ParsingResultSelectionListener {
    ...
    public PreviewPanel(Project project) {
        ...
        project.getMessageBus().connect().subscribe(AntlrListener.Companion.getTOPIC(), new PreViewListener(this));
    }
    ...
    private ActionToolbar createButtonBar() {
        ...
        ActionToolbar toolbar = ActionManager.getInstance().createActionToolbar("antlr.pre.view.window", actionGroup, false);
    ...
    private static Pair<UberTreeViewer, JPanel> createParseTreePanel() {
        // wrap tree and slider in a panel
    ...
    public void grammarFileSaved(VirtualFile grammarFile) {
        ...
        if (previewState.getG() != null && previewState.getStartRuleName() != null) {
            updateParseTreeFromDoc(previewState.getGrammarFile());
        } else {
    ...
    private void ensureStartRuleExists(VirtualFile grammarFile) {
        ...
        if (previewState.getG() != null &&
                ...
                previewState.getStartRuleName() != null) {
            Rule rule = previewState.getG().getRule(previewState.getStartRuleName());
            if (rule == null) {
                previewState.setStartRuleName(null);
    ...
    private void switchToGrammar(VirtualFile grammarFile) {
        ...
        setEnabled(previewState.getG() != null || previewState.getLg() == null);
    }
    ...
    protected void autoSetStartRule(PreviewState previewState) {
        if (previewState.getG() == null || previewState.getG().rules.isEmpty()) {
            // If there is no grammar all of a sudden, we need to unset the previous rule name
            previewState.setStartRuleName(null);
        } else if (previewState.getStartRuleName() == null) {
            OrderedHashMap<String, Rule> rules = previewState.getG().rules;
            previewState.setStartRuleName(rules.getElement(0).name);
        }
    }
    ...
    private void setEnabledRecursive(Component component, boolean enabled) {
        if (component instanceof JTable) {
            // it seems there's a special case
    ...
    private void updateTreeViewer(final PreviewState preview, final ParsingResult result) {
        if (preview.getG() == null) {
            return;
        }
        if (result.parser instanceof PreviewParser) {
            AltLabelTextProvider provider = new AltLabelTextProvider(result.parser, preview.getG());
            if (buildTree) {
            ...
            if (buildTree) {
                treeViewer.setRuleNames(Arrays.asList(preview.getG().getRuleNames()));
                treeViewer.setTree(result.tree);
            }
            if (buildHierarchy) {
                hierarchyViewer.setRuleNames(Arrays.asList(preview.getG().getRuleNames()));
                hierarchyViewer.setTree(result.tree);
            }
        }
    }
    ...
    public void updateParseTreeFromDoc(VirtualFile grammarFile) {
        ...
        updateQueue.queue(new Update(this) {
            ...
            public void run() {
                ...
                antlrService.parseText(grammarFile, inputText);
            }
        });
    }
    ...
    public void onParsingCompleted(PreviewState previewState, long duration) {
        ApplicationManager.getApplication().invokeLater(() -> { // make sure we're on GUI thread for this block
            PropertiesComponent.getInstance(project).setValue("ANTLR_PARSE_GRAMMAR_CANCEL_ID", false);
    ...
    public void notifySlowParsing() {
        PropertiesComponent.getInstance(project).setValue("ANTLR_PARSE_GRAMMAR_CANCEL_ID", true);
        buttonBar.updateActionsImmediately();
    }

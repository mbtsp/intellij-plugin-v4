...
import com.antlr.language.psrsing.PreviewInterpreterRuleContext;
import com.antlr.preview.PreviewState;
import com.antlr.ui.view.AltLabelTextProvider;
import com.antlr.ui.view.TrackpadZoomingTreeView;
import com.antlr.util.Utils;
...
public class ShowAmbigTreesDialog extends JDialog {
    ...
    public static void popupAmbigTreesDialog(PreviewState previewState, AmbiguityInfo ambigInfo) {
        if(previewState == null || previewState.parsingResult==null) {return;}
        ...
        int startRuleIndex = parser.getRuleIndex(previewState.getStartRuleName());
        List<ParserRuleContext> ambiguousParseTrees = null;
        try {
            ambiguousParseTrees =
                    GrammarParserInterpreter.getAllPossibleParseTrees(previewState.getG(),
                            parser,
            ...
            // should be no errors for ambiguities unless the original
    ...
    public static JBPopup createLookaheadTreesPopup(final PreviewState previewState,
        ...
        list.add("Show all lookahead interpretations");
        JBPopupFactory factory = JBPopupFactory.getInstance();
        IPopupChooserBuilder<String> builder = factory.createPopupChooserBuilder(list);

        builder.setSelectionMode(ListSelectionModel.SINGLE_SELECTION)
                .setItemChosenCallback(s -> popupLookaheadTreesDialog(previewState, lookaheadInfo));
        return builder.createPopup();
    }
    ...
    public static void popupLookaheadTreesDialog(PreviewState previewState, LookaheadEventInfo lookaheadInfo) {
        ...
        List<ParserRuleContext> lookaheadParseTrees =
                GrammarParserInterpreter.getLookaheadParseTrees(previewState.getG(),
                        parser,
                        parser.getTokenStream(),
                        startRuleIndex,
    ...
    public void setTrees(PreviewState previewState,
        ...
        if (ambiguousParseTrees != null) {
            ...
            JBPanel<JBPanel> panelOfTrees = new JBPanel<>();
            ...
            for (int i = 0; i < numTrees; i++) {
                ...
                AltLabelTextProvider treeText =
                        new AltLabelTextProvider(previewState.parsingResult.parser, previewState.getG());
                ...
                treeViewers[i].addHighlightedNodes(singletonList(ParsingUtils.INSTANCE.findOverriddenDecisionRoot(ctx)));
                if (ctx != chosenTree) {
                    mark(chosenTree, ctx);
                }
                JBPanel<JBPanel> wrapper = new JBPanel<>(new BorderLayout());
    ...
    public static void mark(final PreviewInterpreterRuleContext t,
        ...
        List<TerminalNode> tleaves = ParsingUtils.INSTANCE.getAllLeaves(t);
        List<TerminalNode> uleaves = ParsingUtils.INSTANCE.getAllLeaves(u);
        ...
        for (int i = 0; i < n; i++) { // for each leaf in t and u
            ...
            List<? extends Tree> tancestors = ParsingUtils.INSTANCE.getAncestors(tleaf);
            List<? extends Tree> uancestors = ParsingUtils.INSTANCE.getAncestors(uleaf);
            int a = 0;

...
class CancelParserAction:AnAction() {
    override fun actionPerformed(p0: AnActionEvent) {
        p0.project?.let { project ->
            AntlrService.getInstance(project).cancelParsing()
        }
    }
    override fun update(p0: AnActionEvent) {
        if(p0.project==null) return
        p0.presentation.isEnabled=PropertiesComponent.getInstance(p0.project!!).getBoolean("ANTLR_PARSE_GRAMMAR_CANCEL_ID")
    }

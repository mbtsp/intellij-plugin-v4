...
class AntlrSyntaxHighlighter : SyntaxHighlighterBase() {
    ...
    override fun getTokenHighlights(tokenType: IElementType?): Array<out TextAttributesKey?> {
        ...
        when (tokenType) {
            ...
            AntlrTokenTypes.TOKEN_ELEMENT_TYPES?.get(ANTLRv4Lexer.RULE_REF) -> {
                return pack(RULE_NAME)
            }
            AntlrTokenTypes.TOKEN_ELEMENT_TYPES?.get(ANTLRv4Lexer.STRING_LITERAL), AntlrTokenTypes.TOKEN_ELEMENT_TYPES?.get(ANTLRv4Lexer.UNTERMINATED_STRING_LITERAL) -> {
                return STRING_KEYS
            }
            AntlrTokenTypes.TOKEN_ELEMENT_TYPES?.get(ANTLRv4Lexer.BLOCK_COMMENT) -> {
                return COMMENT_KEYS
            }
            AntlrTokenTypes.TOKEN_ELEMENT_TYPES?.get(ANTLRv4Lexer.DOC_COMMENT) -> {
                return COMMENT_KEYS
            }
            AntlrTokenTypes.TOKEN_ELEMENT_TYPES?.get(ANTLRv4Lexer.LINE_COMMENT) -> {
                return COMMENT_KEYS
            }
            AntlrTokenTypes.BAD_TOKEN_TYPE -> {
                return BAD_CHARKEYS
            }
            else -> {
                return EMPTY_KEYS
